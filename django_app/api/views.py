...
# ページ数を返す
@login_required(login_url='/admin/login/')
def plast(request, mode='all'):
  if mode == 'myposts':
    msgs = Message2.objects.filter(owner=request.user)
  elif mode == 'goods':
    msgs = Message2.objects.filter(good2__owner=request.user)
  else:
    msgs = Message2.objects.all()
...
# 自分がGoodした投稿をJSONで送信する
@login_required(login_url='/admin/login/')
def usr_goods(request, page=1):
  msgs = Message2.objects.filter(good2__owner=request.user)
  paginate = Paginator(msgs, page_max)
  page_items = paginate.get_page(page)
  serialized_data = serialize('json', page_items)
